...
export interface LandingBlock {
  ...
  features_layout?: 'grid'|'list'|'cards'
  features_preset?: 'grid_center'|'grid_hover'|'row_left'|'row_flat'|'big_icon'|'minimal'
...
function FaqItemEl({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div style={{
      ...
      borderRadius:'16px 16px 16px 0', overflow:'hidden', width:'100%', boxSizing:'border-box' as const,
...
function FeaturesGridBlock({ block, isRtl }: { block: LandingBlock; isRtl: boolean }) {
  ...
  const itemTitle = (f: NonNullable<LandingBlock['features']>[number]) => isRtl ? (f.ar||f.en) : (f.en||f.ar)
  const itemSub   = (f: NonNullable<LandingBlock['features']>[number]) => isRtl ? (f.subtitle_ar||f.subtitle_en) : (f.subtitle_en||f.subtitle_ar)
  return (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {features.map((f,i) => (
            <div key={i} className="pk-gold-hover group flex flex-col items-center text-center gap-3 p-5 rounded-2xl border border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800">
              {f.icon_url && <div className="rounded-2xl flex items-center justify-center shadow-sm p-2" style={{background:'linear-gradient(135deg,#fef3c7,#fde68a)'}}><FeatureIcon icon_url={f.icon_url} size={f.icon_size||40}/></div>}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {features.map((f,i) => (
            <div key={i} className="pk-gold-hover group flex flex-col items-center text-center gap-3 p-5 rounded-2xl border-2 border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-800 hover:!bg-amber-500 hover:!border-amber-400 cursor-default transition-colors">
              {f.icon_url && <div className="rounded-xl flex items-center justify-center bg-amber-50 dark:bg-amber-500/20 group-hover:bg-white/20 transition-colors p-2"><FeatureIcon icon_url={f.icon_url} size={f.icon_size||40}/></div>}
        ...
        <div className="space-y-3">
          {features.map((f,i) => (
            <div key={i} className="pk-gold-hover flex items-center gap-4 p-4 rounded-2xl border border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/60">
              {f.icon_url && <div className="rounded-full flex items-center justify-center flex-shrink-0 shadow-sm p-2" style={{background:'linear-gradient(135deg,#fef3c7,#fde68a)'}}><FeatureIcon icon_url={f.icon_url} size={f.icon_size||36}/></div>}
              <div>
        ...
        <div className="space-y-2">
          {features.map((f,i) => (
            <div key={i} className="pk-gold-hover flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-gray-800/40 border border-transparent">
              {f.icon_url && <div className="rounded-lg flex items-center justify-center flex-shrink-0 bg-white dark:bg-gray-700 border border-gray-100 dark:border-gray-600 p-1.5"><FeatureIcon icon_url={f.icon_url} size={f.icon_size||32}/></div>}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {features.map((f,i) => (
            <div key={i} className="pk-gold-hover flex flex-col items-center text-center gap-4 p-7 rounded-3xl border-2 border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800">
              {f.icon_url && <div className="rounded-full flex items-center justify-center flex-shrink-0 border-2 border-amber-200 dark:border-amber-600/40 p-3" style={{background:'linear-gradient(135deg,#fef9ee,#fef3c7)'}}><FeatureIcon icon_url={f.icon_url} size={f.icon_size||56}/></div>}
              <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
          {features.map((f,i) => (
            <div key={i} className="pk-gold-hover flex flex-col items-center text-center gap-2.5 rounded-2xl border border-transparent p-4">
              {f.icon_url && <div className="rounded-2xl flex items-center justify-center border border-amber-200 dark:border-amber-700/50 bg-amber-50 dark:bg-amber-500/10 p-2"><FeatureIcon icon_url={f.icon_url} size={f.icon_size||32}/></div>}
...
function HowToWorkBlock({ block, isRtl }: { block: LandingBlock; isRtl: boolean }) {
  ...
  const getStep = (s: NonNullable<LandingBlock['hiw_steps']>[number]) => ({
  ...
  const SectionHeader = () => (
    <div style={{ textAlign: titleAlign as any, marginBottom: 32 }}>
      {helper && (
        <span style={{ display:'inline-flex', alignItems:'center', gap:6, fontSize:12, fontWeight:700, color:helperColor, background:helperColor+'18', border:`1px solid ${helperColor}40`, borderRadius:999, padding:'4px 14px', marginBottom:12 }}>
  ...
  const wrapStyle: React.CSSProperties = { borderRadius:20, padding:'36px 24px', position:'relative', overflow:'hidden', background: bgImage ? 'transparent' : sectionBg }
  if (v === 1) return (
    <section style={wrapStyle}>
      ...
      <div style={{ position:'relative', zIndex:2 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))', gap:20 }}>
          {steps.map((s, i) => { const { title: st, desc: sd } = getStep(s); return (
            <div key={i} className="pk-gold-hover" style={{ background: bgImage?'rgba(255,255,255,.10)':'#fff', backdropFilter:bgImage?'blur(8px)':undefined, borderRadius:18, padding:'28px 20px', border:`1px solid ${bgImage?'rgba(255,255,255,.18)':'#f0f0f5'}`, position:'relative' }}>
              <div style={{ width:44, height:44, borderRadius:14, background:accent, display:'flex', alignItems:'center', justifyContent:'center', fontSize:18, fontWeight:900, color:'#fff', marginBottom:16, boxShadow:`0 4px 14px ${accent}50` }}>{i + 1}</div>
      ...
      <div style={{ position:'relative', paddingLeft: isRtl?0:32, paddingRight: isRtl?32:0 }}>
        ...
        {steps.map((s, i) => { const { title: st, desc: sd } = getStep(s); return (
          <div key={i} style={{ display:'flex', gap:20, alignItems:'flex-start', marginBottom: i<steps.length-1?32:0, position:'relative' }}>
            <div style={{ position:'absolute', [isRtl?'right':'left']:-32, top:4, width:28, height:28, borderRadius:'50%', background:accent, display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:900, color:'#fff', flexShrink:0, boxShadow:`0 0 0 4px ${accent}25` }}>{i + 1}</div>
          ...
          <div key={i} style={{ flex:'1 1 160px', display:'flex', alignItems:'flex-start', minWidth:0 }}>
            <div style={{ flex:1, textAlign:'center', padding:'0 12px' }}>
              <div style={{ width:64, height:64, borderRadius:'50%', background:`${accent}18`, border:`2px solid ${accent}`, display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 16px', position:'relative' }}>
                {s.image_url ? <img src={s.image_url} alt="" style={{ width:36, height:36, objectFit:'contain' }}/> : <span style={{ fontSize:22, fontWeight:900, color:accent }}>{i+1}</span>}
            ...
            {i < steps.length - 1 && <div style={{ paddingTop:30, color:`${accent}80`, fontSize:20, flexShrink:0 }}>{isRtl ? '←' : '→'}</div>}
          </div>
        )})}
      </div>
    </section>
  ...
  if (v === 4) return (
    <section style={{ ...wrapStyle, background: sectionBg }}>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:24 }}>
        {steps.map((s, i) => { const { title: st, desc: sd } = getStep(s); const imgLeft = i % 2 === 0; return (
          <div key={i} style={{ display:'flex', flexWrap:'wrap', gap:20, alignItems:'center', flexDirection: imgLeft?'row':'row-reverse' }}>
            {s.image_url && <div style={{ flex:'0 0 auto', width:'clamp(120px,35%,220px)', borderRadius:16, overflow:'hidden', boxShadow:'0 4px 20px rgba(0,0,0,.08)' }}><img src={s.image_url} alt="" style={{ width:'100%', display:'block', objectFit:'cover', aspectRatio:'4/3' }}/></div>}
        ...
        {steps.map((s, i) => { const { title: st, desc: sd } = getStep(s); return (
          <div key={i} className="pk-gold-hover" style={{ background:'#fff', borderRadius:20, padding:'28px 20px', textAlign:'center', border:'1px solid #f0f0f5', boxShadow:'0 2px 16px rgba(0,0,0,.06)', display:'flex', flexDirection:'column', alignItems:'center', gap:12 }}>
            <div style={{ width:64, height:64, borderRadius:18, background:`linear-gradient(135deg,${accent}20,${accent}08)`, border:`1.5px solid ${accent}30`, display:'flex', alignItems:'center', justifyContent:'center', marginBottom:4 }}>
              {s.image_url ? <img src={s.image_url} alt="" style={{ width:40, height:40, objectFit:'contain' }}/> : <span style={{ fontSize:24, fontWeight:900, color:accent }}>{i+1}</span>}
            </div>
...
function ContentBlock({ block, isRtl }: { block: LandingBlock; isRtl: boolean }) {
  const title      = isRtl ? (block.title_ar || block.title_en) : (block.title_en || block.title_ar)
  const desc       = isRtl ? (block.body_ar  || block.body_en)  : (block.body_en  || block.body_ar)
  ...
  const textSide = (
    <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 16, padding: '8px 0' }}>
      {helper && (
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 700, color: helperColor, background: helperColor + '18', borderRadius: 999, padding: '5px 14px', width: 'fit-content', border: `1px solid ${helperColor}40`, alignSelf: titleAlign === 'center' ? 'center' : titleAlign === 'right' ? 'flex-end' : 'flex-start' }}>
          <span style={{ width: 7, height: 7, borderRadius: '50%', background: helperColor, display: 'inline-block' }}/>{helper}
        </span>
      )}
      {title && <h2 style={{ fontSize: 'clamp(22px,4vw,34px)', fontWeight: 800, color: '#111827', margin: 0, lineHeight: 1.25, textAlign: titleAlign as any }}>{title}</h2>}
      {desc && <p style={{ fontSize: 15, color: descColor, lineHeight: 1.75, margin: 0, textAlign: descAlign as any }}>{desc}</p>}
      {stats.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 12, marginTop: 4 }}>
          {stats.map((s, i) => {
            ...
            return (
              <div key={i} className="pk-gold-hover" style={{ background: '#f9fafb', borderRadius: 14, padding: '14px 16px', border: '1px solid #f0f0f5' }}>
                <p style={{ fontSize: 28, fontWeight: 800, color: '#d99401', margin: 0, lineHeight: 1 }}>{s.value}{s.suffix || ''}</p>
      ...
      {btnText && (
        <div style={{ marginTop: 8 }}>
          <a href={btnLink} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: btnBg, color: '#fff', fontSize: 14, fontWeight: 700, padding: '12px 28px', borderRadius: 12, textDecoration: 'none', transition: 'filter .2s, transform .2s' }}
  ...
  const imgSideEl = imgUrl ? (
    <div style={{ flex: '0 0 auto', width: 'clamp(160px,40%,320px)', borderRadius: 20, overflow: 'hidden', alignSelf: 'stretch' }}>
      {imgLink ? <a href={imgLink} target="_blank" rel="noopener noreferrer" style={{ display: 'block', height: '100%' }}><img src={imgUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', minHeight: 200 }}/></a>
  ...
  return (
    <section style={{ borderRadius: 20, overflow: 'hidden', background: '#ffffff', border: '1px solid #f0f0f5', boxShadow: '0 4px 24px rgba(0,0,0,.06)' }}>
      <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: 0, minHeight: 260 }}>
        ...
        <div style={{ flex: 1, minWidth: 200, padding: '32px 28px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>{textSide}</div>
...
function StatsBlock({ block, isRtl }: { block: LandingBlock; isRtl: boolean }) {
  ...
  useEffect(() => {
    ...
    const duration = 1800; const startTime = performance.now(); const targets = items.map(it => it.value || 0)
    const easeOut = (t: number) => 1 - Math.pow(1 - t, 3)
    const frame = (now: number) => {
      const progress = Math.min((now - startTime) / duration, 1); const eased = easeOut(progress)
  ...
  return (
    <section ref={refEl} style={{ background: sectionBg, borderRadius: 20, padding: '32px 20px' }}>
      {title && (
        <h2 style={{ fontSize:'clamp(20px,4vw,28px)', fontWeight:800, textAlign:'center', color:'#101010', margin:'0 0 28px', display:'flex', alignItems:'center', justifyContent:'center', gap:16 }}>
          <span style={{ width:40, height:3, background:'#101010', borderRadius:2, display:'block' }}/>{title}<span style={{ width:40, height:3, background:'#101010', borderRadius:2, display:'block' }}/>
        </h2>
      )}
      <div style={{ display:'grid', gridTemplateColumns:`repeat(auto-fill,minmax(${cardMinW}px,1fr))`, gap:20 }}>
        {items.map((it, i) => {
          ...
          return (
            <div key={i} className="pk-gold-hover" style={{ background:cardBg, borderRadius:24, padding:`${cardPad}px 20px`, textAlign:'center', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:8, boxShadow:'0 4px 20px rgba(0,0,0,.06)', border:'1px solid transparent' }}>
              <p style={{ fontSize:numSize, fontWeight:700, color:numColor, lineHeight:1, margin:0, fontVariantNumeric:'tabular-nums' }}>{counts[i].toLocaleString()}{it.suffix || ''}</p>
...
function CountdownBlock({ block, isRtl }: { block: LandingBlock; isRtl: boolean }) {
  ...
  useEffect(() => {
    ...
    const getEnd = () => {
      try { const stored = localStorage.getItem(key); if (stored) { const end = parseInt(stored, 10); if (end > Date.now()) return end } } catch {}
    ...
    const tick = () => {
      ...
      if (diff === 0) { endTime = Date.now() + hours * 3600000; try { localStorage.setItem(key, String(endTime)) } catch {} }
      const total = Math.floor(diff / 1000)
      setTimeLeft({ d: Math.floor(total / 86400), h: Math.floor((total % 86400) / 3600), m: Math.floor((total % 3600) / 60), s: total % 60 })
    }
    tick(); const id = setInterval(tick, 1000); return () => clearInterval(id)
  }, [block.id, hours])
  const pad = (n: number) => String(n).padStart(2, '0')
  const labels = isRtl ? ['يوم', 'ساعة', 'دقيقة', 'ثانية'] : ['Days', 'Hours', 'Minutes', 'Seconds']
  ...
  return (
    <section style={{ padding: '12px 0' }}>
      <div style={{ display:'flex', justifyContent:'center', alignItems:'center', gap:8, flexWrap:'wrap', padding:12, borderRadius:10, background:`${boxBg} radial-gradient(rgba(255,255,255,.08) 1px, transparent 1px)`, backgroundSize:'12px 12px', border:'1px solid rgba(255,255,255,.08)' }}>
...
function BannersBlock({ block }: { block: LandingBlock }) {
  ...
  const Img = ({ i, style }: { i: number; style?: React.CSSProperties }) => {
    const src = imgs[i]?.image_url; const href = imgs[i]?.link_url
    const el = src ? <img src={src} alt="" style={{ width:'100%', height:'100%', objectFit:'cover', display:'block', borderRadius:r }}/> : <div style={{ width:'100%', height:'100%', background:'#e8e8f0', borderRadius:r, display:'flex', alignItems:'center', justifyContent:'center', color:'#9ca3af', fontSize:13 }}>Image {i+1}</div>
  ...
  if (v === 1) return <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap }}>{[0,1].map(i=><Img key={i} i={i} style={{ aspectRatio:'4/3' }}/>)}</div>
  if (v === 2) return <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap }}><Img i={0} style={{ gridRow:'span 2' }}/><Img i={1} style={{ aspectRatio:'4/3' }}/><Img i={2} style={{ aspectRatio:'4/3' }}/></div>
  if (v === 3) return <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap }}>{[0,1,2,3].map(i=><Img key={i} i={i} style={{ aspectRatio:'1' }}/>)}</div>
  if (v === 4) return <div style={{ display:'flex', flexDirection:'column', gap }}><Img i={0} style={{ aspectRatio:'21/9', width:'100%' }}/><div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap }}>{[1,2,3,4].map(i=><Img key={i} i={i} style={{ aspectRatio:'1' }}/>)}</div></div>
  if (v === 5) return <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap }}><Img i={0} style={{ gridRow:'span 3' }}/><Img i={1} style={{ aspectRatio:'4/3' }}/><Img i={2} style={{ aspectRatio:'4/3' }}/><Img i={3} style={{ aspectRatio:'4/3' }}/></div>
  if (v === 6) return <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap }}>{[0,1,2].map(i=><Img key={i} i={i} style={{ aspectRatio:'3/4' }}/>)}</div>
  if (v === 7) return <div style={{ display:'flex', flexDirection:'column', gap }}><div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap }}><Img i={0} style={{ aspectRatio:'16/9' }}/><Img i={1} style={{ aspectRatio:'16/9' }}/></div><div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap }}>{[2,3,4].map(i=><Img key={i} i={i} style={{ aspectRatio:'1' }}/>)}</div></div>
  if (v === 8) return <div style={{ display:'flex', flexDirection:'column', gap }}><Img i={0} style={{ aspectRatio:'21/9', width:'100%' }}/><div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap }}>{[1,2,3].map(i=><Img key={i} i={i} style={{ aspectRatio:'1' }}/>)}</div></div>
  if (v === 9) return <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gridTemplateRows:'auto auto', gap }}><Img i={0} style={{ gridRow:'span 2' }}/><Img i={1} style={{ aspectRatio:'16/9' }}/><div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap }}><Img i={2} style={{ aspectRatio:'1' }}/><Img i={3} style={{ aspectRatio:'1' }}/></div></div>
  return null
}

function TestimonialsBlock({ block, isRtl }: { block: LandingBlock; isRtl: boolean }) {
  ...
  const hoverCSS = `.${blockId}-card { transition: background .2s, box-shadow .2s, transform .2s; } .${blockId}-card:hover { ${hoverBg ? `background: ${hoverBg} !important;` : ''} transform: translateY(-3px); box-shadow: 0 12px 32px rgba(0,0,0,.08); } ${tc.hover_text_color ? `.${blockId}-card:hover .tst-review { color: ${tc.hover_text_color} !important; }` : ''} ${tc.author_name_color_hover ? `.${blockId}-card:hover .tst-name { color: ${tc.author_name_color_hover} !important; }` : ''} ${tc.review_heading_color_hover ? `.${blockId}-card:hover .tst-heading { color: ${tc.review_heading_color_hover} !important; }` : ''}`
  const SectionHeader = () => (
    <>
  ...
  const Avatar = ({ r, size=48, rounded='50%', i=0 }: { r: typeof reviews[0]; size?: number; rounded?: string|number; i?: number }) => (
    r.author_image
      ? <img src={r.author_image} alt={r.author_name} style={{ width:size, height:size, borderRadius:rounded, objectFit:'cover', flexShrink:0 }}/>
      : <div style={{ width:size, height:size, borderRadius:rounded, background: v===8 ? avatarColors[i%avatarColors.length] : '#fde68a', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', fontSize:size*0.4 }}>👤</div>
  )
  if (v === 1) return (
    <section style={{ padding:'32px 0' }}>
      <style>{hoverCSS}</style><SectionHeader/>
        ...
        {reviews.map((r,i) => (
          <div key={i} className={`pk-gold-hover ${blockId}-card`} style={{ background:cardBg, borderRadius:16, padding:24, boxShadow:'0 2px 12px rgba(0,0,0,.06)', border:'1px solid #f0f0f5' }}>
            <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:14 }}>
              <Avatar r={r} size={48}/>
              <div><div className="tst-name" style={{ fontWeight:700, fontSize:15, color:nameColor }}>{r.author_name}</div><StarRow/></div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))', gap:20 }}>
        {reviews.map((r,i) => (
          <div key={i} className={`pk-gold-hover ${blockId}-card`} style={{ background:cardBg, borderRadius:20, padding:'28px 24px', boxShadow:'0 4px 20px rgba(0,0,0,.07)', textAlign:'center', border:'1px solid #f0f0f5', position:'relative' }}>
            ...
            <div style={{ display:'flex', justifyContent:'center', gap:8, marginTop:8 }}><StarRow/>{r.type==='google'?<GoogleIcon/>:<FacebookIcon/>}</div>
          </div>
        ))}
      </div>
    </section>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(240px,1fr))', gap:20 }}>
        {reviews.map((r,i) => (
          <div key={i} className={`pk-gold-hover ${blockId}-card`} style={{ background:cardBg, borderRadius:20, padding:28, textAlign:'center', boxShadow:'0 2px 16px rgba(0,0,0,.06)', border:'1px solid #f0f0f5' }}>
            <Avatar r={r} size={64} i={i}/>
            <div style={{ display:'flex', justifyContent:'center', marginBottom:12, marginTop:14 }}><StarRow/></div>
          ...
          <div key={i} className={`pk-gold-hover ${blockId}-card`} style={{ background:'#fff', borderRadius:16, padding:24, boxShadow:'0 4px 24px rgba(0,0,0,.08)' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14 }}>
              <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                <Avatar r={r} size={44} i={i}/>
                <div><div className="tst-name" style={{ fontWeight:700, fontSize:14, color:nameColor }}>{r.author_name}</div><StarRow/></div>
              </div>
              {r.type==='google'?<GoogleIcon/>:<FacebookIcon/>}
            </div>
            <p className="tst-review" style={{ color:revColor, fontSize:14, lineHeight:'1.7', margin:0 }}>{r.review}</p>
        ...
        {reviews.map((r,i) => (
          <div key={i} className={`pk-gold-hover ${blockId}-card`} style={{ background:tc.bg_color||'#fff', borderRadius:16, padding:24, marginBottom:20, breakInside:'avoid', boxShadow:'0 4px 20px rgba(0,0,0,.07)', border:'1px solid #f0f0f5' }}>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
              <Avatar r={r} size={42} i={i}/>
              <div><div className="tst-name" style={{ fontWeight:700, fontSize:13, color:nameColor }}>{r.author_name}</div><div style={{ display:'flex', gap:4, alignItems:'center', marginTop:2 }}><StarRow/>{r.type==='google'?<GoogleIcon/>:<FacebookIcon/>}</div></div>
            </div>
            <p className="tst-review" style={{ color:revColor, fontSize:14, lineHeight:'1.7', margin:0 }}>{r.review}</p>
          </div>
        ))}
        ...
        {reviews.map((r,i) => (
          <div key={i} className={`pk-gold-hover ${blockId}-card`} style={{ background:cardBg, borderRadius:20, padding:24, boxShadow:'0 4px 20px rgba(0,0,0,.07)', border:'1px solid #f0f0f5' }}>
            <div style={{ display:'flex', gap:10, alignItems:'center', marginBottom:14 }}>
              <Avatar r={r} size={48} rounded={12} i={i}/>
              <div><div className="tst-name" style={{ fontWeight:700, fontSize:14, color:nameColor }}>{r.author_name}</div><StarRow/></div>
            </div>
        ...
        {reviews.map((r,i) => (
          <div key={i} className={`pk-gold-hover ${blockId}-card`} style={{ background:tc.bg_color||'#1a1a2e', borderRadius:20, padding:28, boxShadow:'0 4px 24px rgba(0,0,0,.2)', border:'1px solid rgba(217,148,1,.2)' }}>
            <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:16 }}>
              ...
              <div><div className="tst-name" style={{ fontWeight:700, fontSize:15, color:tc.author_name_color||'#fff' }}>{r.author_name}</div><StarRow/></div>
              <div style={{ marginLeft:'auto' }}>{r.type==='google'?<GoogleIcon/>:<FacebookIcon/>}</div>
            </div>
...
// ── Main Renderer ──────────────────────────────────────────────────────────
export default function LandingBlocksRenderer({ blocks, isRtl = false }: { blocks: LandingBlock[]; isRtl?: boolean }) {
  useArabicFont(isRtl)
  useEffect(() => { injectGoldShimmer() }, [])
  if (!blocks || blocks.length === 0) return null
  return (
    <div className="space-y-14" dir={isRtl ? 'rtl' : 'ltr'} style={isRtl ? { fontFamily: "'Cairo', sans-serif" } : {}}>
      {blocks.map(block => {
        const title = isRtl ? (block.title_ar||block.title_en) : (block.title_en||block.title_ar)
        const body  = isRtl ? (block.body_ar||block.body_en)  : (block.body_en||block.body_ar)

        if (block.layout === 'features_grid') return <FeaturesGridBlock key={block.id} block={block} isRtl={isRtl}/>
        if (block.layout === 'how_to_work')   return <HowToWorkBlock   key={block.id} block={block} isRtl={isRtl}/>
        if (block.layout === 'content')       return <ContentBlock     key={block.id} block={block} isRtl={isRtl}/>
        if (block.layout === 'stats')         return <StatsBlock       key={block.id} block={block} isRtl={isRtl}/>
        if (block.layout === 'countdown')     return <CountdownBlock   key={block.id} block={block} isRtl={isRtl}/>
        if (block.layout === 'banners')       return <BannersBlock     key={block.id} block={block}/>
        if (block.layout === 'testimonials')  return <TestimonialsBlock key={block.id} block={block} isRtl={isRtl}/>
              ...
              {(block.cards||[]).map((c,i) => {
                ...
                return (
                  <div key={i} className="pk-gold-hover group rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 transition-all">
                    {c.image_url
                      ? <div className="aspect-square overflow-hidden bg-gray-50 dark:bg-gray-800"><img src={c.image_url} alt={cardTitle||''} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"/></div>
                      : <div className="aspect-square bg-gradient-to-br from-amber-50 to-amber-100 dark:from-amber-900/20 dark:to-amber-800/10 flex items-center justify-center"><span className="text-4xl opacity-30">🖼</span></div>
                    }
                    <div className="p-4">
        ...
        if (block.layout === 'marquee') {
          ...
          return (
            <section key={block.id} style={{ background: bg, overflow: 'hidden', padding: '15px 0' }}>
              <style>{`@keyframes pk-marquee-${block.id}{0%{transform:translateX(0)}100%{transform:translateX(-50%)}}.pk-marquee-${block.id}{display:flex;flex-direction:row;flex-wrap:nowrap;gap:32px;align-items:center;animation:pk-marquee-${block.id} ${speed}s linear infinite;width:max-content;}`}</style>
        ...
        if (block.layout === 'video') {
          ...
          return (
            <section key={block.id}>
              ...
              {vUrl && (
                <div className="relative rounded-3xl p-4 md:p-7" style={{ background: 'linear-gradient(135deg, #fef9ee 0%, #fef3c7 40%, #fde68a 100%)' }}>
                  ...
                  <div className="relative rounded-2xl overflow-hidden shadow-2xl aspect-video">
                    <iframe src={vUrl} className="w-full h-full" allowFullScreen frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"/>
                  </div>
                </div>
              )}
            </section>
        ...
        if (block.layout === 'faq') return (
          <section key={block.id} style={{ backgroundImage:`url(https://files.easy-orders.net/1730815545147228170serv-bg.svg)`, backgroundSize:'cover', backgroundPosition:'center', backgroundColor:'#fdf7e8', borderRadius:16, padding:'32px 0', overflow:'hidden' }}>
            {title && (
              <>
                <style>{`@keyframes pk-faq-pulse{0%{transform:scale(1)}50%{transform:scale(1.01)}100%{transform:scale(1)}}@keyframes pk-faq-grad{0%{background-position:0%}50%{background-position:200%}100%{background-position:0%}}.pk-faq-desc{text-align:center;font-size:clamp(22px,4vw,40px);font-weight:bold;margin:0 0 15px;word-break:break-word;background:linear-gradient(90deg,#d99401,#f5c76b,#d99401);background-size:200%;-webkit-background-clip:text;background-clip:text;color:transparent !important;animation:pk-faq-grad 5s ease-in-out infinite,pk-faq-pulse 1.5s ease-in-out infinite;}`}</style>
                <p className="pk-faq-desc">{title}</p>
              </>
            )}
            {body && (
              <>
                <style>{`.pk-faq-badge{font-weight:800;text-align:center;line-height:130%;text-transform:uppercase;padding:14px 24px;border-radius:30px;font-size:14px;color:#52443a;background-color:rgba(217,148,1,0.15);width:fit-content;margin:0 auto 20px;letter-spacing:2px;position:relative;z-index:1;border:4px solid #d99401;display:block;background-clip:padding-box;}`}</style>
            ...
            <div style={{ padding:'0 12px', width:'100%', boxSizing:'border-box', maxWidth:800, margin:'0 auto' }}>
              {(block.faqs||[]).map((faq,i) => (
                <FaqItemEl key={i} q={isRtl?(faq.q_ar||faq.q_en):(faq.q_en||faq.q_ar)} a={isRtl?(faq.a_ar||faq.a_en):(faq.a_en||faq.a_ar)}/>
              ))}
            </div>
          </section>
        )

        if (block.layout === 'html') return block.html_code ? <div key={block.id} dangerouslySetInnerHTML={{ __html: block.html_code }}/> : null
