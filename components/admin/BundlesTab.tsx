...
function SectionCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="border border-gray-100 dark:border-gray-800 rounded-xl mb-3">
      <div className="bg-gray-50 dark:bg-gray-800/60 border-b border-gray-100 dark:border-gray-800 border-l-2 border-l-amber-400 px-3.5 py-2 rounded-t-xl">
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <button onClick={openAdd} className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold transition-colors">
      ...
      {bundles.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-600 text-white text-sm font-bold transition-colors">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {bundles.map(b => (
            <div key={b.id} className="rounded-2xl overflow-hidden bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-sm">
              <div className="h-1 w-full" style={{ background: b.is_active ? '#d99401' : '#D1D5DB' }} />
              <div className="p-4">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    {b.image_url
                      ? <img src={b.image_url} alt={b.name} className="w-9 h-9 object-contain rounded-lg" />
                      : <div className="w-9 h-9 rounded-lg bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center"><Layers size={16} className="text-amber-400" /></div>}
                    <div>
                      <div className="text-sm font-bold text-gray-900 dark:text-gray-100">{b.name}</div>
                      <div className="text-xs text-amber-600 dark:text-amber-400 font-semibold mt-0.5">{b.price_egp.toLocaleString()} EGP</div>
                    </div>
                  </div>
      ...
      {/* Add/Edit Modal */}
      {modal && (
        <div className="fixed inset-0 bg-black/50 dark:bg-black/70 z-50 flex items-center justify-center p-4" style={{ backdropFilter: 'blur(4px)' }} onClick={() => setModal(null)}>
          <div className="rounded-2xl w-full max-w-lg flex flex-col shadow-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800" style={{ maxHeight: '90vh', minHeight: 0 }} onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 flex-shrink-0 border-b border-gray-100 dark:border-gray-800">
              <h2 className="text-sm font-bold text-gray-900 dark:text-gray-100">{modal === 'add' ? 'New Bundle' : 'Edit Bundle'}</h2>
            ...
            <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-0">
              <SectionCard title="Details">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-1.5">Bundle Name *</label>
                    <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Student Pack" className={inp} />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-1.5">Price (EGP)</label>
              ...
              <SectionCard title={`Select Tools (${form.selectedIds.length} selected)`}>
                        ...
                        {group.tools.map(t => {
                          ...
                          return (
                            <button key={t.id} onClick={() => toggleTool(t.id)}
                              className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold text-left transition-all border ${
                                selected
                                  ? 'bg-amber-50 dark:bg-amber-500/10 border-amber-400 text-amber-700 dark:text-amber-400'
                              ...
                              {selected && <Check size={11} className="ml-auto flex-shrink-0 text-amber-500" />}
                            </button>
                          )
                        })}
                      </div>
            ...
            <div className="flex gap-2 px-5 pb-5 pt-3 flex-shrink-0 border-t border-gray-100 dark:border-gray-800">
              ...
              <button onClick={save} disabled={saving}
                className="flex-[2] py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 disabled:opacity-60 text-white text-xs font-bold transition-colors flex items-center justify-center gap-2">
                {saving ? <div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><Check size={13} />{modal === 'add' ? 'Create Bundle' : 'Save Changes'}</>}
              </button>
            </div>
          </div>
        </div>
      ...
      {/* Delete confirm */}
      {delId && (
        <div className="fixed inset-0 bg-black/50 dark:bg-black/70 z-50 flex items-center justify-center p-4" style={{ backdropFilter: 'blur(4px)' }} onClick={() => setDelId(null)}>
          <div className="rounded-2xl w-full max-w-sm p-6 shadow-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800" onClick={e => e.stopPropagation()}>
            ...
            <h3 className="text-sm font-bold text-gray-900 dark:text-gray-100 text-center mb-1">Delete Bundle?</h3>
