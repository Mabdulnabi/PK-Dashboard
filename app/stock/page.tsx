    ...
    <div className="flex h-screen bg-gray-950 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-auto p-6">
          ...
          {/* Low stock warning */}
          {lowTools.length>0 && (
            <div className="mb-5 flex items-start gap-3 px-4 py-3 rounded-xl bg-amber-900/30 border border-amber-700/50">
          ...
          <div className="flex items-stretch gap-4 mb-6">
            <div className="flex-1 grid grid-cols-3 gap-4">
              {[
                { label:'إجمالي الحسابات', val:stock.length,     color:'text-gray-200', bg:'#1F2937' },
                { label:'متاح',            val:totalAvail,       color:'text-emerald-400', bg:'#0D1F18' },
                { label:'مُخصص',           val:totalAssigned,    color:'text-blue-400',  bg:'#0D1628' },
              ].map(s=>(
            ...
            <button onClick={openAdd} className="px-5 rounded-2xl bg-red-500 hover:bg-red-600 text-white font-bold text-sm flex items-center gap-2 transition-colors">
          ...
          <div className="flex gap-2 mb-4 flex-wrap">
            <button onClick={()=>setSelTool('all')}
              className={`px-4 py-2 rounded-xl text-sm font-semibold transition-colors ${selTool==='all'?'bg-red-500 text-white':'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>
              الكل
            </button>
            {tools.map(t=>(
              <button key={t.id} onClick={()=>setSelTool(t.id)}
                className={`px-4 py-2 rounded-xl text-sm font-semibold transition-colors flex items-center gap-2 ${selTool===t.id?'bg-red-500 text-white':'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>
                {t.image_url && <img src={t.image_url} alt={t.name} className="w-4 h-4 object-contain rounded"/>}
                {t.name}
                <span className={`text-xs font-bold px-1.5 py-0.5 rounded-md ${t.available<3?'bg-amber-500/20 text-amber-400':'bg-gray-700 text-gray-400'}`}>
                  {t.available}
                </span>
              </button>
            ))}
          ...
          <div className="rounded-2xl overflow-hidden" style={{border:'1px solid #1F2937', background:'#111827'}}>
            {loading ? (
              ...
              <div className="text-center py-16 text-gray-500">
                <Archive size={32} className="mx-auto mb-3 text-gray-700"/>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {filtered.map(s=>{
                    ...
                    return (
                      <tr key={s.id} style={{borderBottom:'1px solid #1F2937'}} className="hover:bg-gray-800/30 transition-colors">
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-2">
                            {tool?.image_url && <img src={tool.image_url} alt={tool.name} className="w-6 h-6 object-contain rounded"/>}
                            <span className="text-sm text-gray-300">{tool?.name||s.tool_id}</span>
                          </div>
                        </td>
                        <td className="px-5 py-4 text-sm text-gray-300 font-mono" dir="ltr">{s.email}</td>
                        <td className="px-5 py-4 text-xs text-gray-500">{s.notes||'—'}</td>
                        <td className="px-5 py-4">
                          <span className={`inline-flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-lg ${s.status==='available'?'bg-emerald-500/15 text-emerald-400':'bg-blue-500/15 text-blue-400'}`}>
                        ...
                        <td className="px-5 py-4">
                          {s.members ? (
                            <div>
                              <div className="text-xs text-gray-300">{(s.members as any).full_name}</div>
                        ...
                        <td className="px-5 py-4">
                          <button onClick={()=>deleteItem(s.id)} disabled={s.status==='assigned'||deleting===s.id}
                            className="w-7 h-7 rounded-lg flex items-center justify-center transition-colors disabled:opacity-30 disabled:cursor-not-allowed bg-red-500/10 hover:bg-red-500/20 text-red-400">
                            {deleting===s.id
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={()=>setAddModal(false)}>
          <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-md p-6 shadow-2xl" onClick={e=>e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-base font-bold text-white">إضافة حساب للمخزون</h2>
              <button onClick={()=>setAddModal(false)} className="w-7 h-7 rounded-lg bg-gray-800 flex items-center justify-center text-gray-400 hover:text-white">
                <X size={14}/>
              </button>
            </div>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1.5 block">الأداة</label>
                <select value={form.tool_id} onChange={e=>setForm(f=>({...f,tool_id:e.target.value}))}
                  className="w-full px-4 py-3 text-sm rounded-xl bg-gray-800 border border-gray-700 text-white outline-none focus:border-red-500 transition-colors">
                  {tools.map(t=>(
                ...
                <div className="flex gap-2">
                  {(['account','key'] as const).map(type=>(
                    <button key={type} onClick={()=>setForm(f=>({...f,delivery_type:type}))}
                      className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition-colors ${form.delivery_type===type?'bg-red-500 text-white':'bg-gray-700 text-gray-400 hover:bg-gray-600'}`}>
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={()=>setAddModal(false)} className="flex-1 py-3 rounded-xl border border-gray-700 text-sm font-semibold text-gray-400 hover:bg-gray-800 transition-colors">
                إلغاء
              </button>
