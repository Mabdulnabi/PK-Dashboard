...
function HexBadge({ rk, size = 72, active = false }: { rk: typeof RANKS[number]; size?: number; active?: boolean }) {
  const s = size / 72
  const cx = 36, cy = 36, r = 32, ri = 22

  const hex = (radius: number, dx = 0, dy = 0) => {
    const h = radius * 0.866
    return `${cx+radius+dx},${cy+dy} ${cx+radius/2+dx},${cy+h+dy} ${cx-radius/2+dx},${cy+h+dy} ${cx-radius+dx},${cy+dy} ${cx-radius/2+dx},${cy-h+dy} ${cx+radius/2+dx},${cy-h+dy}`
  }

  const icons: Record<RankKey, JSX.Element> = {
    regular: (
      <>
        <circle cx={cx} cy={cy-4} r="4" fill={rk.light} opacity="0.9"/>
        <path d={`M${cx-6},${cy+8} Q${cx-6},${cy+1} ${cx},${cy+1} Q${cx+6},${cy+1} ${cx+6},${cy+8}`} fill={rk.light} opacity="0.9"/>
      </>
    ),
    bronze: (
      <>
        <polygon points={`${cx},${cy-10} ${cx+8.7},${cy-5} ${cx+8.7},${cy+5} ${cx},${cy+10} ${cx-8.7},${cy+5} ${cx-8.7},${cy-5}`} fill="none" stroke={rk.light} strokeWidth="2"/>
        <circle cx={cx} cy={cy} r="3.5" fill={rk.light}/>
      </>
    ),
    silver: (
      <>
        <circle cx={cx} cy={cy} r="10" fill={rk.mid}/>
        <circle cx={cx} cy={cy} r="6" fill={rk.light}/>
      </>
    ),
    gold: (
      <>
        <polygon points={`${cx},${cy-10} ${cx+8.7},${cy-5} ${cx+8.7},${cy+5} ${cx},${cy+10} ${cx-8.7},${cy+5} ${cx-8.7},${cy-5}`} fill={rk.color}/>
        <polygon points={`${cx},${cy-6} ${cx+5.2},${cy-3} ${cx+5.2},${cy+3} ${cx},${cy+6} ${cx-5.2},${cy+3} ${cx-5.2},${cy-3}`} fill={rk.light}/>
      </>
    ),
    platinum: (
      <path d={`M${cx},${cy-11} L${cx+2.9},${cy-4} L${cx+10.5},${cy-3.7} L${cx+4.3},${cy+1.4} L${cx+6.5},${cy+8.9} L${cx},${cy+4.5} L${cx-6.5},${cy+8.9} L${cx-4.3},${cy+1.4} L${cx-10.5},${cy-3.7} L${cx-2.9},${cy-4} Z`} fill={rk.light} opacity="0.95"/>
    ),
    emerald: (
      <>
        <polygon points={`${cx},${cy-11} ${cx+7},${cy-3.5} ${cx+9.5},${cy+5} ${cx},${cy+10} ${cx-9.5},${cy+5} ${cx-7},${cy-3.5}`} fill={rk.color}/>
        <polygon points={`${cx},${cy-7} ${cx+4},${cy-1} ${cx+5.5},${cy+4} ${cx},${cy+7} ${cx-5.5},${cy+4} ${cx-4},${cy-1}`} fill={rk.light}/>
      </>
    ),
    diamond: (
      <>
        <polygon points={`${cx},${cy-12} ${cx+11},${cy} ${cx},${cy+12} ${cx-11},${cy}`} fill={rk.light}/>
        <polygon points={`${cx},${cy-12} ${cx+11},${cy} ${cx},${cy-2}`} fill="white" opacity="0.65"/>
        <polygon points={`${cx-11},${cy} ${cx},${cy-2} ${cx},${cy+12}`} fill={rk.dark} opacity="0.5"/>
      </>
    ),
  }

  return (
    <svg width={size} height={size} viewBox="0 0 72 72" style={{filter: active ? `drop-shadow(0 0 8px ${rk.color}88)` : undefined}}>
      {/* shadow */}
      <polygon points={hex(r, 2, 3)} fill="#000" opacity="0.25"/>
      {/* bevel facets */}
      <polygon points={`${cx+r/2},${cy-r*.866} ${cx+r},${cy} ${cx+ri},${cy} ${cx+ri/2},${cy-ri*.866}`} fill={rk.light}/>
      <polygon points={`${cx-r/2},${cy-r*.866} ${cx+r/2},${cy-r*.866} ${cx+ri/2},${cy-ri*.866} ${cx-ri/2},${cy-ri*.866}`} fill={rk.light} opacity="0.85"/>
      <polygon points={`${cx+r},${cy} ${cx+r/2},${cy+r*.866} ${cx+ri/2},${cy+ri*.866} ${cx+ri},${cy}`} fill={rk.mid}/>
      <polygon points={`${cx-r},${cy} ${cx-r/2},${cy-r*.866} ${cx-ri/2},${cy-ri*.866} ${cx-ri},${cy}`} fill={rk.mid} opacity="0.85"/>
      <polygon points={`${cx+r/2},${cy+r*.866} ${cx-r/2},${cy+r*.866} ${cx-ri/2},${cy+ri*.866} ${cx+ri/2},${cy+ri*.866}`} fill={rk.dark}/>
      <polygon points={`${cx-r/2},${cy+r*.866} ${cx-r},${cy} ${cx-ri},${cy} ${cx-ri/2},${cy+ri*.866}`} fill={rk.darkest}/>
      {/* inner dark ring */}
      <circle cx={cx} cy={cy} r={ri} fill="#0a0e14"/>
      <circle cx={cx} cy={cy} r={ri-3} fill={rk.darkest}/>
      <circle cx={cx} cy={cy} r={ri-7} fill="#1a2030"/>
      {/* icon */}
      {icons[rk.key]}
    </svg>
  )
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-6 items-start">
        ...
        <div className="flex flex-col gap-6">
          <div className="rounded-2xl overflow-hidden" style={glassCard}>
            {/* Avatar hero */}
            <div className="relative px-8 py-8 flex items-center gap-6" style={{background:'linear-gradient(135deg,#0d1117 0%,#1a1200 100%)'}}>
        ...
        {/* ── Right: Rank + Preferences ── */}
        <div className="flex flex-col gap-5">

          {/* ── Rank card ── */}
          <div className="rounded-2xl overflow-hidden" style={glassCard}>
            {/* current rank hero */}
            <div className="px-5 py-5 flex items-center gap-4" style={{background:`linear-gradient(135deg, ${rank.darkest}ee 0%, #0d111a 100%)`}}>
              <HexBadge rk={rank} size={72} active/>
              <div className="flex-1 min-w-0">
                <p className="text-xs text-gray-400 mb-0.5">{t('Your rank','رتبتك الحالية')}</p>
                <p className="text-lg font-bold" style={{color: rank.light}}>{lang==='ar' ? rank.ar : rank.en}</p>
                <p className="text-xs mt-1" style={{color: rank.color}}>
                  {t('Total spent','إجمالي الإنفاق')}: {spent.toLocaleString()} {t('EGP','جنيه')}
                </p>
              </div>
            </div>
            ...
            {nextRank && (
              <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-800/60">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-gray-500">{t('Next','الرتبة التالية')}: <span className="font-bold" style={{color: nextRank.color}}>{lang==='ar' ? nextRank.ar : nextRank.en}</span></span>
                  <span className="text-xs text-gray-400">{(nextRank.min - spent).toLocaleString()} {t('EGP remaining','جنيه متبقي')}</span>
                </div>
            ...
            {!nextRank && (
              <div className="px-5 py-3 border-b border-gray-100 dark:border-gray-800/60">
                <p className="text-xs text-center font-bold" style={{color: rank.color}}>🏆 {t('Maximum rank achieved!','وصلت للرتبة الأعلى!')}</p>
              </div>
            )}

            {/* all ranks row */}
            <div className="px-5 py-4">
              <p className="text-[10px] text-gray-400 mb-3 uppercase tracking-wide">{t('All ranks','كل الرتب')}</p>
              <div className="flex items-end justify-between gap-1">
                {RANKS.map((r, i) => {
                  const active = r.key === rank.key
                  const unlocked = i <= rankIdx
                  return (
                    <div key={r.key} className="flex flex-col items-center gap-1">
                      <div style={{opacity: unlocked ? 1 : 0.35, transform: active ? 'scale(1.15)' : 'scale(1)', transition:'transform .2s'}}>
                        <HexBadge rk={r} size={active ? 40 : 32}/>
                      </div>
                      <span className="text-[8px] font-bold" style={{color: active ? r.light : unlocked ? r.color : '#6b7280'}}>
                        {lang==='ar' ? r.ar : r.en}
                      </span>
                    </div>
                  )
