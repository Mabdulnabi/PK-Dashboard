...
function TypeBadge({ slug }: { slug: string }) {
  const map: Record<string, { label: string; color: string; Icon: any }> = {
    shared:  { label: 'Shared',  color: '#3b82f6', Icon: Users },
    bundle:  { label: 'Bundle',  color: '#f59e0b', Icon: Package },
    private: { label: 'Private', color: '#8b5cf6', Icon: LockKey },
  }
  const m = map[slug] || map.shared
  const Icon = m.Icon
  return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold"
      style={{ background: m.color + '18', color: m.color }}>
      <Icon size={10} weight="duotone"/>{m.label}
    </span>
  )
}
...
export default function CartPage() {
  ...
  const checkout = async () => {
    ...
    if (cart.length === 1 && cart[0].quantity === 1) {
      const t = cart[0].shop_tools
      router.push(`/u/checkout?tool_id=${t.id}`)
      return
    }
    // For multi-item carts, direct to checkout with first item (TODO: multi-cart checkout)
          ...
          <div className="flex-1 space-y-3">
            {cart.length === 0 ? (
              <div className="flex flex-col items-center gap-4 py-20 text-center">
                ...
                <Link href="/u/shop" className="px-5 py-2.5 rounded-xl text-white text-sm font-bold transition-colors" style={{ background: GOLD }}>
              ...
              <>
                {cart.map(item => {
                  ...
                  return (
                    <div key={item.id} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-4 flex items-start gap-4">
                      {/* Image */}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            ...
                            <div className="mt-1">
                              <TypeBadge slug={tool?.category_slug || 'shared'}/>
                            </div>
                          </div>
                        ...
                        <div className="flex items-center justify-between mt-3 flex-wrap gap-2">
                          {/* Qty controls — private only */}
                          {isPrivate ? (
                            <div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-800 rounded-lg px-2 py-1">
                              <button onClick={() => updateQty(tool.id, item.quantity - 1)} disabled={item.quantity <= 1}
                              ...
                              <button onClick={() => updateQty(tool.id, item.quantity + 1)}
                            ...
                            <span className="text-xs text-gray-400">{t('Qty: 1 (fixed)', 'الكمية: 1')}</span>
                          )}

                          {/* Actions */}
                          <div className="flex items-center gap-2">
                            <button onClick={() => { toggleFav(tool.id); showToast(isFav(tool.id) ? t('Removed from favorites','تمت الإزالة من المفضلة') : t('Added to favorites ♥','تمت الإضافة للمفضلة ♥')) }}
                              ...
                              style={{ color: isFav(tool.id) ? '#ef4444' : '#9ca3af' }}>
                              <Heart size={13} weight={isFav(tool.id) ? 'fill' : 'regular'}/>{t('Fav','مفضلة')}
                            </button>
                            <button onClick={() => { removeFromCart(tool.id); showToast(t('Removed','تمت الإزالة')) }}
              ...
              <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5 sticky top-6">
                ...
                <div className="space-y-2 mb-4">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-500">{t('Items', 'عناصر')} ({cartCount})</span>
                    <span className="font-semibold text-gray-800 dark:text-gray-200">{fmt(subtotal)}</span>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {favorites.length === 0 ? (
            ...
            favorites.map(fav => {
              ...
              const alreadyInCart = cart.some(i => i.tool_id === tool?.id)
              return (
                <div key={fav.id} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-4 flex flex-col">
                  ...
                  <p className="text-base font-bold mb-3" style={{ color: GOLD }}>{fmt(tool?.price_egp || 0)}</p>
                  <div className="flex gap-2 mt-auto">
                    <button onClick={() => { toggleFav(tool.id); showToast(t('Removed from favorites','تمت الإزالة من المفضلة')) }}
                    ...
                    <button
                      onClick={async () => {
                        await addToCart(tool.id, 1)
                        showToast(t('Added to cart ✓', 'تمت الإضافة للسلة ✓'))
                      }}
                      disabled={alreadyInCart}
                      className="flex-1 py-2 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-1.5 disabled:opacity-60"
                      style={alreadyInCart
