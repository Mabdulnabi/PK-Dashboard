...
function AdminReplyBox({ reviewId, onReply, S }: { reviewId: string; onReply: (rep:any)=>void; S: any }) {
  ...
  if (!open) return (
    <button onClick={()=>setOpen(true)} style={{marginTop:8, fontSize:11, color:S.amber, background:'none', border:'none', cursor:'pointer', fontWeight:600}}>
      + Reply as Support
    </button>
  )
  return (
    <div style={{marginTop:8, display:'flex', gap:8}}>
      <input value={text} onChange={e=>setText(e.target.value)}
        ...
        style={{flex:1, fontSize:12, padding:'6px 10px', borderRadius:8, border:`1px solid ${S.border}`, background:S.surface, color:S.text1, outline:'none'}}
        onKeyDown={e=>e.key==='Enter'&&send()}/>
      <button onClick={send} disabled={sending||!text.trim()}
        style={{fontSize:11, fontWeight:700, color:'#fff', background:'#d99401', border:'none', borderRadius:8, padding:'6px 12px', cursor:'pointer', opacity:sending?0.5:1}}>
        {sending?'…':'Send'}
      </button>
      <button onClick={()=>setOpen(false)} style={{fontSize:11, color:S.text3, background:'none', border:'none', cursor:'pointer'}}>Cancel</button>
    </div>
  )
}
