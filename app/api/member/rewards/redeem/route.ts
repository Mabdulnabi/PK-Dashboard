...
export async function POST() {
  ...
  try {
    ...
    let code = genCode()
    for (let i = 0; i < 5; i++) {
      const { data: existing } = await service.from('coupons').select('id').eq('code', code).maybeSingle()
      if (!existing) break
      code = genCode()
    }
    ...
    if (couponErr || !coupon) {
      ...
      return NextResponse.json({ error: couponErr?.message ?? 'insert_failed' }, { status: 500 })
    }

    // Deduct points
    const nowIso = now.toISOString()
    await service.from('loyalty_points').update({
      balance: balance - points_to_redeem,
