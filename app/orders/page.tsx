...
import {
  ...
  Search, Download, CheckSquare, Square, Pencil, CreditCard, Tag,
} from 'lucide-react'
...
const CAT_STYLES: Record<string, { label: string; light: string; dark: string; stripe: string }> = {
  shared:  { label:'Shared',  light:'bg-blue-100 text-blue-700',    dark:'dark:bg-blue-900/20 dark:text-blue-400',    stripe:'#3b82f6' },
  private: { label:'Private', light:'bg-purple-100 text-purple-700', dark:'dark:bg-purple-900/20 dark:text-purple-400', stripe:'#8b5cf6' },
...
export default function OrdersPage() {
  ...
  const [bulkForm,     setBulkForm]     = useState({ delivery_type:'account' as 'account'|'key', email:'', password:'', key:'', notes:'', extend_days:'' })
  ...
  const deleteDelivery = async () => {
    if (!modal || !window.confirm(`Clear delivery for ${modal.member_name}?`)) return
    setDeleting(true)
    await fetch(`/api/admin/orders/${modal.id}/deliver`, { method: 'DELETE' })
    setDeleting(false)
    setToast({ msg: 'Delivery cleared', type: 'ok' })
    setModal(null)
    load()
  }
  ...
  // Bulk edit — update delivery for all selected private orders
  const runBulkEdit = async () => {
    const privateSelected = orders.filter(o => selected.has(o.id) && o.category_slug === 'private')
    if (privateSelected.length === 0) { setToast({msg:'No private orders selected',type:'err'}); return }

    setBulkSaving(true)
    const delivPromises = privateSelected.map(o =>
      fetch(`/api/admin/orders/${o.id}/deliver`, {
        method: 'POST', headers: {'Content-Type':'application/json'},
        body: JSON.stringify({
          delivery_type: bulkForm.delivery_type,
          email:    bulkForm.email    || undefined,
          password: bulkForm.password || undefined,
          key:      bulkForm.key      || undefined,
          notes:    bulkForm.notes    || undefined,
        })
      })
    )

    const extendDays = parseInt(bulkForm.extend_days)
    const expiryPromises = !isNaN(extendDays) && extendDays > 0
      ? privateSelected.map(o => {
          const base = o.expires_at ? new Date(o.expires_at) : new Date()
          base.setDate(base.getDate() + extendDays)
          return fetch(`/api/admin/orders/${o.id}`, {
    ...
    await Promise.all([...delivPromises, ...expiryPromises])
    setBulkSaving(false)
    setBulkModal(false)
    setSelected(new Set())
    setToast({msg:`✓ Updated ${privateSelected.length} orders`,type:'ok'})
    load()
  }

  // Filtered list
  const filtered = orders.filter(o => {
    if (memberFilter && o.member_id !== memberFilter) return false
    ...
    if (search) {
      ...
      if (!o.member_name?.toLowerCase().includes(q) &&
          ...
          !(o.member_code||'').toLowerCase().includes(q)) return false
    }
    return true
  })
  ...
  const selectedPrivate = orders.filter(o=>selected.has(o.id)&&o.category_slug==='private').length
    ...
    <div className="flex h-screen bg-gray-50 dark:bg-[#0D1117] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-auto p-6">
          ...
          {/* Member filter chip */}
          {memberFilter && (
            <div className="flex items-center gap-2 mb-3 px-3 py-2 rounded-xl bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-700/40 w-fit">
              <User size={13} className="text-indigo-500"/>
              <span className="text-xs font-bold text-indigo-700 dark:text-indigo-300">
                {orders.find(o=>o.member_id===memberFilter)?.member_name}
              </span>
              <button onClick={()=>setMemberFilter(null)} className="text-indigo-400 hover:text-indigo-600 ms-1"><X size={11}/></button>
            </div>
          )}
          ...
          <div className="flex flex-wrap items-center gap-2 mb-4">
            <div className="relative flex-1 min-w-[200px] max-w-xs">
              ...
              <input value={search} onChange={e=>{setSearch(e.target.value);setPage(1)}}
                placeholder="Search name, email, tool, PK code…"
          ...
          {selected.size > 0 && (
            <div className="flex items-center gap-3 mb-4 px-4 py-3 rounded-xl border border-amber-200 dark:border-amber-500/30 bg-amber-50 dark:bg-amber-500/10">
              <span className="text-sm font-bold text-amber-700 dark:text-amber-400">
                ...
                {selectedPrivate > 0 && ` (${selectedPrivate} private)`}
              </span>
              {selectedPrivate > 0 && (
                <button onClick={()=>{
                  setBulkForm({ delivery_type:'account', email:'', password:'', key:'', notes:'', extend_days:'' })
                  setBulkModal(true)
                }}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold text-white"
                  style={{background:'#6366f1'}}>
                  <Pencil size={12}/> Bulk Edit Private
                </button>
              )}
              ...
              <button onClick={()=>setSelected(new Set())} className="ms-auto text-xs text-gray-500 hover:text-gray-700 dark:hover:text-gray-300">
                Clear
              </button>
            </div>
          )}
                ...
                {paged.map(o=>{
                  ...
                  return (
                    <div key={o.id}
                      ...
                      <div className="h-1 w-full" style={{background:cat.stripe}}/>
                      ...
                      <div className="mx-4 mb-2 px-3 py-2 rounded-xl bg-gray-50 dark:bg-gray-800/60 border border-gray-100 dark:border-gray-700/50">
                        <div className="flex items-center justify-between mb-0.5">
                          <button
                            ...
                            onClick={()=>{ setMemberFilter(memberFilter===o.member_id?null:o.member_id); setPage(1) }}>
          ...
          <div className="bg-white dark:bg-[#111827] border border-gray-100 dark:border-[#1a2233] rounded-2xl w-full max-w-md p-6 shadow-2xl" onClick={e=>e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">{modal.tool_name}</p>
              </div>
            ...
            <div className="mb-4 px-3 py-2.5 rounded-xl bg-gray-50 dark:bg-gray-800/60 border border-gray-100 dark:border-gray-700">
              <div className="flex items-center justify-between">
                ...
                {modal.member_code && (
                  <span className="text-[10px] font-mono font-bold px-1.5 py-0.5 rounded bg-gray-200 dark:bg-gray-700 text-gray-500">{modal.member_code}</span>
                )}
              </div>
            ...
            <div className="space-y-4">
              ...
              {form.delivery_type==='account'?(<>
                <div>
                  ...
                  <input value={form.email} onChange={e=>setForm(f=>({...f,email:e.target.value}))} placeholder="user@example.com" dir="ltr" className={inp}/>
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5 block">Password</label>
                  <div className="relative">
                    <input value={form.password} onChange={e=>setForm(f=>({...f,password:e.target.value}))} type={showPass?'text':'password'} placeholder="••••••••" dir="ltr" className={inp+' pr-10'}/>
                    <button onClick={()=>setShowPass(s=>!s)} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">{showPass?<EyeOff size={15}/>:<Eye size={15}/>}</button>
                  </div>
                </div>
              </>):(
                <div>
                  ...
                  <div className="relative">
                    <input value={form.key} onChange={e=>setForm(f=>({...f,key:e.target.value}))} type={showPass?'text':'password'} placeholder="XXXX-XXXX-XXXX" dir="ltr" className={inp+' pr-10 font-mono'}/>
                    <button onClick={()=>setShowPass(s=>!s)} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">{showPass?<EyeOff size={15}/>:<Eye size={15}/>}</button>
                  </div>
                </div>
              )}
            ...
            <div className="flex gap-3 mt-5">
              {modal?.delivered && (
                <button onClick={deleteDelivery} disabled={deleting}
                  className="px-4 py-3 rounded-xl border border-red-200 dark:border-red-500/30 text-sm font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 disabled:opacity-50">
                  {deleting?'…':'🗑 Clear'}
                </button>
              )}
              <button onClick={()=>setModal(null)} className="flex-1 py-3 rounded-xl border border-gray-200 dark:border-gray-700 text-sm font-semibold text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800">
                Cancel
              </button>
              <button onClick={deliver} disabled={saving||formLoading}
                className="flex-[2] py-3 rounded-xl disabled:opacity-60 text-white text-sm font-bold flex items-center justify-center gap-2"
                style={{background:GOLD}}>
      ...
      {bulkModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={()=>setBulkModal(false)}>
          <div className="bg-white dark:bg-[#111827] border border-gray-100 dark:border-[#1a2233] rounded-2xl w-full max-w-md p-6 shadow-2xl" onClick={e=>e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="text-base font-bold text-gray-900 dark:text-white">Bulk Edit Private Orders</h2>
                <p className="text-xs text-gray-400 mt-0.5">{selectedPrivate} private orders selected · leave blank to skip that field</p>
              </div>
              <button onClick={()=>setBulkModal(false)} className="w-7 h-7 rounded-lg bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-gray-400"><X size={14}/></button>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  {(['account','key'] as const).map(type=>(
                    <button key={type} onClick={()=>setBulkForm(f=>({...f,delivery_type:type}))}
                      className="flex-1 py-2.5 rounded-xl text-sm font-bold transition-colors"
              ...
              {bulkForm.delivery_type==='account'?(<>
                <div>
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5 block">Email</label>
                  <input value={bulkForm.email} onChange={e=>setBulkForm(f=>({...f,email:e.target.value}))} placeholder="(blank = keep existing)" dir="ltr" className={inp}/>
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5 block">Password</label>
                  <input value={bulkForm.password} onChange={e=>setBulkForm(f=>({...f,password:e.target.value}))} placeholder="(blank = keep existing)" dir="ltr" className={inp}/>
                </div>
              </>):(
                <div>
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5 block">Key</label>
                  <input value={bulkForm.key} onChange={e=>setBulkForm(f=>({...f,key:e.target.value}))} placeholder="(blank = keep existing)" dir="ltr" className={inp+' font-mono'}/>
                </div>
              )}
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5 block">Notes</label>
                <input value={bulkForm.notes} onChange={e=>setBulkForm(f=>({...f,notes:e.target.value}))} placeholder="(blank = keep existing)" className={inp}/>
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5 block">Extend Duration (days)</label>
                <input value={bulkForm.extend_days} onChange={e=>setBulkForm(f=>({...f,extend_days:e.target.value}))} placeholder="e.g. 30 (blank = no change)" type="number" min="1" dir="ltr" className={inp}/>
                <p className="text-[10px] text-gray-400 mt-1">Adds days to each order's current expiry date</p>
              </div>
            </div>

            <div className="flex gap-3 mt-5">
              <button onClick={()=>setBulkModal(false)} className="flex-1 py-3 rounded-xl border border-gray-200 dark:border-gray-700 text-sm font-semibold text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800">
                Cancel
              </button>
              <button onClick={runBulkEdit} disabled={bulkSaving}
                className="flex-[2] py-3 rounded-xl disabled:opacity-60 text-white text-sm font-bold flex items-center justify-center gap-2"
                style={{background:'#6366f1'}}>
                {bulkSaving?<><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"/>Updating…</>:<><Check size={15}/>Apply to {selectedPrivate} Orders</>}
              </button>
            </div>
          </div>
        </div>
