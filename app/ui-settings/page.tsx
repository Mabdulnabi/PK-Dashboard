...
export default function UISettingsPage() {
  ...
  const inp = "w-full bg-[#1F2937] text-white text-sm rounded-lg px-3 py-2.5 border border-[#374151] focus:border-blue-500 outline-none transition-colors"

  function LogoSlot({ slot }: { slot: typeof LOGO_SLOTS[number] }) {
    const url  = urls[slot.key] || ''
    const busy = uploadingSlot === slot.key
    return (
      <div className="flex items-start gap-4 py-4 border-b border-[#374151] last:border-b-0">
        {/* Preview */}
        <div className="w-16 h-16 rounded-xl bg-[#111827] border border-[#374151] flex items-center justify-center overflow-hidden flex-shrink-0">
          {url
        ...
        <div className="flex-1 min-w-0">
          <div className="text-sm font-semibold text-white mb-0.5">{slot.label}</div>
          ...
          {url && (
            <input
              ...
              className="mt-2 w-full bg-[#111827] text-gray-400 text-xs rounded-lg px-3 py-2 border border-[#374151] focus:border-blue-500 outline-none transition-colors"
            />
          )}
        </div>
      </div>
        ...
        <div className="flex-1 overflow-auto p-6">
          {loading ? (
            ...
            <div className="max-w-2xl space-y-6">
              ...
              {/* ── Member Portal Logos ──────────────────── */}
              <div style={{ background: '#1F2937', border: '1px solid #374151' }} className="rounded-2xl p-6">
                <div className="flex items-center gap-2 mb-1">
                  <Key size={14} className="text-blue-400"/>
                  <h2 className="text-white font-bold text-sm">لوجوهات بورتال الأعضاء</h2>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-[#374151]">
                  <div>
              ...
              <div style={{ background: '#1F2937', border: '1px solid #374151' }} className="rounded-2xl p-6">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h2 className="text-white font-bold text-sm">لوجوهات لوحة الأدمن</h2>
                </div>
                <p className="text-xs text-gray-500 mb-4">
